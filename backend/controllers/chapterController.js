...
export const acceptJoinRequest = async (req, res) => {
        try {
                ...
                await chapter.save();
                await users.findByIdAndUpdate(userId, { groupJoined: true });

                res.status(200).json({ message: "User added to members" });
        } catch (error) {
...
export const removeMemberFromChapter = async (req, res) => {
        try {
                ...
                // Update user's groupJoined to false
                await users.findByIdAndUpdate(userId, { groupJoined: false });
